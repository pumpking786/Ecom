  ...
  storeValidate = (data, created_by) => {
    try {
      let schema = Joi.object({
        ...
        created_by: Joi.string().required(), // Add created_by as required ObjectId string
      });
  ...
  updateValidate = (data) => {
    try {
      let schema = Joi.object({
        ...
        is_featured: Joi.boolean().default(false),
        status: Joi.string().valid("active", "inactive").default("active"),
        // Removed created_by from schema
      }).unknown(true); // Allow unknown fields like created_by
      ...
      console.error("Validation error details:", err.message, err.stack);
      throw new Error(err.message || "Validation error occurred");
    }
  };
}
